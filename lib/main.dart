...
Future<void> _firebaseMessagingBackgroundHandler(RemoteMessage message) async {
  ...
  print('_firebaseMessagingBackgroundHandler called..  ${message.notification?.body}');
  ...
  if (global.sp!.getString("currentUser") != null) {
    ...
    print("Notification chekc----------> ${messageData}");
    if (message.notification!.title == "For starting the timer in other audions for video and audio") {
      ...
      print("notificatoin chekc-----> ${message.notification!.title}");
      Future.delayed(Duration(milliseconds: 500)).then((value) async {
        await _localNotifications.cancelAll();
      });
  ...
  void initState() {
    ...
    FirebaseMessaging.onMessage.listen((RemoteMessage message) async {
      ...
      if (message.notification!.title == "For Live accept/reject") {
        ...
        print("notificatoin chekc----->1 ${message.notification!.title}");
        chatController.showBottomAcceptChat = false;
        global.sp = await SharedPreferences.getInstance();
        global.sp!.remove('chatBottom');
        global.sp!.setInt('chatBottom', 0);
    ...
    // callController.getAstrologerDetails(64);
  }
