...
class CallController extends GetxController with GetSingleTickerProviderStateMixin {
  ...
  Future<void> acceptedCall(int callId) async {
    try {
      await global.checkBody().then((result) async {
        if (result) {
          await apiHelper.acceptCall(callId).then((result) {
            print("Result acceptedCall --> $result");
            if (result.status == "200") {
              global.showToast(
  ...
  getAstrologerDetails(int astrologerId)async{
    print("Inside--> getAstrologerDetails()");
    try {
      await global.checkBody().then((result) async {
        if (result) {
          await apiHelper.getAstrologerById(astrologerId,global.user.id).then((result) {
            if (result.status == "200") {
              print("getAstrologerDetails---> $result");
              print("getAstrologerDetails---> ${result.recordList}");
              print("getAstrologerDetails---> ${result.runtimeType}");

              astrologerModel = result.recordList;
              update();


              // print("CAll Charges${astrologerModel?.charge}");
            } else {
              global.showToast(
                message: 'Call Accepet fail',
                textColor: global.textColor,
      ...
      print("Exception getAstrologerDetails:-" + e.toString());
    }
  }
