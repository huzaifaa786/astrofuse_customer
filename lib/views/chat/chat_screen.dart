...
class AcceptChatScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ///Below is the code to find the chat time on the basis of astrologer charge and amount in user wallet.
    // double wallet_amt = walletController.getAmount();
    double wallet_amt = splashController.currentUser?.walletAmount??0;
    // double astrologer_charge = bottomNavigationController.charge??0;
    double astrologer_charge = bottomNavigationController.astrologerbyId.where((element) => element.id == astrologerId).first.charge?.toDouble()??0;
    // print('wallet_amt $wallet_amt');
    // print('astrologer_charge $astrologer_charge');

    int chat_time = ((wallet_amt / astrologer_charge) * 60).round();
                      ...
                      flagId == 1
                          ? CountdownTimer(
                              endTime: DateTime.now().millisecondsSinceEpoch +
                                  1000 * chat_time, //removed the constant 300
                                                              ...
                                                              Column(
                                                                crossAxisAlignment:
                                                                    CrossAxisAlignment
                                                                        .start,
                                                                children: [
                                                                  Text(chatController.reviewData[0].isPublic ==
                                                                              0
                                                                          ? 'Anonymous'
