...
//     getAstrologerDetails(astrologerId:widget.astrologerId).whenComplete((){
// setState(() {
//
// });
  ...
  Future<int> getEndDate({AstrologerModel? astrologerModel}) async {

      try{
        await _callController.getAstrologerDetails(widget.astrologerId);
      }catch(e){
        print('Exception getEndDate() $e');
      }

      int totalBalnce = splashController.currentUser!.walletAmount!.toInt();
      int callCost = _callController.astrologerModel.first.charge??100;
      int callCount = 0;

      while (totalBalnce >= callCost) {
        totalBalnce -= callCost;
        callCount++;
      }
      endTime = DateTime.now().millisecondsSinceEpoch + 1000 *  ( callCount * 60);

      print('endTime Inside Function--> $endTime');
  ...
  void join() async {
    ...
    print("check Sid Now -----> ${global.agoraSid1}--@--${global.agoraSid2}");
  }
